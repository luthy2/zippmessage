...
def find_contacts():
	user = g.user
	resp = twitter.get('friends/ids.json', data = {"screen_name":str(user.username)}, token = get_twitter_token())
	not_contacts=[]
	not_users = []
	c = []
	error = None
	if resp.status == 200:
		...
		for i in range(size):
			_ids = ids[s:(s+100)]
			s += 100
			_r = twitter.post('users/lookup.json', data = {"user_id":_ids}, token = get_twitter_token())
			if _r.status == 200:
				friends = _r.data
				for f in friends:
