...
from models import User, Message, UserMessage, Collection, CollectionItem, Activity, get_url_content
...
def message_reader(message_id):
	user = g.user
	m = Message.query.get(message_id)
	user_m = UserMessage.query.filter(UserMessage.message_id==m.id).filter(UserMessage.user_id).one()
...
@app.route('/collection/<string:unique_id>')
...
def api_create_collection():
	...
	db.session.commit()
	return jsonify(ok=True, id=str(collection.unique_id))
